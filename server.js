...
app.post('/render-reel', async (req, res) => {
  ...
  try {
    ...
    console.log('[' + requestId + '] 2. Rendering Template D in Browserless...');
  const code = `export default async ({ page, context }) => {
    ...
    }, title, imageDataUrl, badgeText, subtitleText);
  ...
  const res = await fetch(BROWSERLESS_URL, {
    ...
    body: JSON.stringify({
      code,
      context: { title, imageDataUrl, badgeText, subtitleText }
    })
  });
    ...
    console.log('[' + requestId + '] 4. Encoding MP4 video with FFmpeg...');
